...
def flexibeam(antenna_positions, doas, widths, lambda_, resolution=1e-4):
    ...
    :param beamwidth: desired beamwidth in radian
    ...
    ampli=np.exp(-2*(np.pi**2)/lambda_**2*np.dot((np.linalg.norm(antenna_positions,axis=1)**2).reshape((-1,1)),(sigma**2).reshape((1,-1))))
